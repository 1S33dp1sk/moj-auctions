...
export function transformRecord(raw, nowISO = new Date().toISOString()) {
  ...
  // Media is attached by the scraper after transform; default to empty for the seed.
  ...
  const announcement = Array.isArray(raw.announcement) ? raw.announcement : [];
  ...
  return {
    ...
    // media (تقرير الخبرة / الصور / الاعلان)
    photos, // الصور — real car photos
    report, // تقرير الخبرة — expert valuation report pages
    announcement, // الاعلان — official sale-notice image(s)
    images: photos, // alias used for the card thumbnail
